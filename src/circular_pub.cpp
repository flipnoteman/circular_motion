...
int main(int argc, char **argv) {
  ...
  ros::init(
      argc, argv,
      "circular_mover"); // Initializes the ros node with arguments from ROS
...
void currentStateCallback(const simulator_utils::Waypoint::ConstPtr &point) {
  ...
  // ROS_INFO("Current Position: [%f, %f, %f]", currentState.x, currentState.y,
  // currentState.z);
}
...
void desiredStateCallback(const circular_motion::State::ConstPtr &state) {
  ...
  if (_id_rec == id) {
    ROS_INFO("Next state for %d = %f, %f, %f", _id_rec, point.x, point.y,
             point.z);
